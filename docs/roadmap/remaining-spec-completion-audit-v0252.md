# v0.2.52 剩余 Spec 完成审计

审计对象：`docs/roadmap/remaining-spec-high-standard-goal.md`

审计结论：不能标记 complete。线上版本已经推进到 `v0.2.52 旧档拦截`，真机记录入口、保存、复制、直达、系统分享、保存后自动分享、最终 JSON 校验器、页面内保存门槛和旧记录导出复查都已完成并部署，但仍缺真实手机浏览器跑完 60 秒后实际发回的 JSON。

## 已复核证据

| 要求 | 当前证据 | 状态 |
| --- | --- | --- |
| 线上正式入口可打开 | `https://games.atou.cc/combo-card-roguelike/versions/a/?v=052-curl-live` 返回 `v0.2.52 旧档拦截` | 已验收 |
| Games Hub 版本已更新 | `https://games.atou.cc/data/games-hub.json?v=052-curl-live` 返回 `score: v0.2.52`、`title: v0.2.52 旧档拦截` | 已验收 |
...
| 公开运行目录干净 | `docs/verification/v0252-mobile-acceptance-stale-record-gates.md` 记录公开 tools 路径 404，运行目录只保留四类文件 | 已验收 |
...
线上 Playwright 手机视口验收显示，浏览器历史里只有 12 秒的旧记录时不能分享；历史设备名是 `local mobile viewport` 时不能复制；设备名是 `iPhone 15 Pro Safari` 且 probe 满 60 秒时会分享 `v0.2.52` JSON；普通入口不会误开真机长测面板。

公开运行目录清理后重新部署，Cloudflare Version ID 为 `43820725-193d-45ea-9292-bfe343667406`。

## 不能完成的原因

最后缺口仍是物理设备证据。当前环境可以证明线上流程、保存门槛、分享 payload、旧记录导出复查、fallback、最终校验器和最终验收 Markdown 生成，但不能产生真实手机的发热、手感、手机浏览器长期运行和用户实际分享记录。必须拿到真实手机跑完 60 秒后发回的 JSON，才能把 active goal 标记完成。
