...
当前版本是 `v0.2.86 全屏滑动出招`。

这一版把战斗输入改成战斗区任意位置滑动出招。方向动作不再要求瞄准某张卡，上划追身、左划破招、右划重刀、下划撤身先固定下来，卡牌暂时保留为构筑和状态展示。

上一版 `v0.2.85 实验台可读性` 修复 Boss 实验战在手机上浅底白字看不清的问题，提高 Boss 卡和实验预设卡可读性，并更新缓存版本避免旧样式残留。
