...
它看起来像卡牌游戏，但目标手感更接近动作游戏。玩家在手机上通过战斗区滑动出招，点按手牌消耗主动技能，观察 Boss 的读招提示，在后摇、连击、破招、追击、撤身和用牌保命之间做选择。

当前线上版本是 `v0.2.89 主动卡牌`。
...
这一版把 4 张手牌接回主动使用。战斗区任意位置滑动仍然是方向出招，点按某张手牌会消耗它并触发回血、防守、拖慢 Boss、补牌加速或爆发伤害。

上一版 `v0.2.88 菜单版本修正` 修正主菜单里版本记录卡片仍显示旧版本的问题，长测记录也改为读取当前版本号，并更新缓存版本让手机拉取新脚本。

`v0.2.87 进战与受击节奏` 修复选关后进战慢的问题，开战不再等待整套资源预热。Boss 命中玩家后会留出一段恢复间隔，标准难度给玩家找回节奏，高难仍保留更快追击。

`v0.2.86 全屏滑动出招` 把战斗输入改成战斗区任意位置滑动出招。方向动作不再要求瞄准某张卡，上划追身、左划破招、右划重刀、下划撤身先固定下来，卡牌暂时保留为构筑和状态展示。

`v0.2.85 实验台可读性` 修复 Boss 实验战在手机上浅底白字看不清的问题，提高 Boss 卡和实验预设卡可读性，并更新缓存版本避免旧样式残留。

`v0.2.84 Boss 实验战` 把 Boss 调试台公开成 Boss 实验战，主菜单可直接进入。新增风暴队长和赤线宿敌实验预设，用来快速测试教学容错、回血护盾、破势爆发和策略压力，并在导出页显示最近实验结果对比。

`v0.2.83 Boss 调试台` 新增手机竖屏 Boss 调试台，可以直达三幕关键 Boss，修改难度参数和成长包，导入导出配置，保存本地预设，并记录每场调试战结果。
